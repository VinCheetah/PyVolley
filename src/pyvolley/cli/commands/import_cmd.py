...
def import_data(
    ...
    if "parse" in steps:
        ...
        if not keep_pdfs:
            getattr(dispatcher, "_cleanup_parsed_pdfs", _cleanup_parsed_pdfs)(saison=saisons, verbose=verbose)
        ...
        with make_progress(console) as progress:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                for match_id, pdf_path, result, parse_error in executor.map(
                    ...
                    # Commit par batch et purge périodique de l'Identity Map SQLAlchemy
