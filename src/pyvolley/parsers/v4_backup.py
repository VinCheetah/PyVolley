...
# Dataclass pour les changements de joueur
...
class SetDetailedInfo:
    ...
    formation_a: Optional[list[str]] = None  # [pos_I, pos_II, ..., pos_VI]
    formation_b: Optional[list[str]] = None
    service_initial: Optional[str] = None  # 'A' ou 'B'
    changements_a: Optional[list] = None  # Liste de (entrant, sortant, score)
    changements_b: Optional[list] = None
    timeouts_a: Optional[list] = None  # Liste de (score_a, score_b)
    timeouts_b: Optional[list] = None
    scores_service_a: Optional[list] = None  # Scores aux tours de service équipe A
    scores_service_b: Optional[list] = None
    
    def __post_init__(self):
        if self.changements_a is None:
            self.changements_a = []
        if self.changements_b is None:
            self.changements_b = []
        if self.timeouts_a is None:
            self.timeouts_a = []
        if self.timeouts_b is None:
            self.timeouts_b = []
        if self.scores_service_a is None:
            self.scores_service_a = []
        if self.scores_service_b is None:
            self.scores_service_b = []
...
class MatchSheetParserV4(BaseParser):
    ...
    def parse(self, pdf_path: Path) -> ParseResult:
        ...
        try:
            ...
            with pdfplumber.open(str(pdf_path)) as pdf:
                ...
                # 6. Parser les liberos et marquer les joueurs correspondants
                ...
                # 7. Parser les officiels (entraîneurs, assistants)
                officiels_a, officiels_b = self._parse_officiels_from_tables(tables)
                
                # 8. Parser les capitaines (numéros entourés)
                capitaine_a, capitaine_b = self._parse_capitaines(tables, full_text)
                self._mark_capitaine(joueurs_a, capitaine_a)
                self._mark_capitaine(joueurs_b, capitaine_b)
                
                # 9. Parser les détails des sets depuis la table principale
                sets_detailed = self._parse_sets_detailed(tables)
                
                # 10. Parser les scores de sets depuis la table RESULTATS
                sets = self._parse_sets_from_tables(tables, resultat, sets_detailed)
                fields_count += len(sets) * 3
                
                # 11. Parser les arbitres et officiels
                arbitres = self._parse_arbitres_from_tables(tables, full_text)
                fields_count += len(arbitres)
                
                # 12. Parser les sanctions (si présentes)
    ...
    def _parse_liberos_from_tables(self, tables: list) -> tuple[list[Joueur], list[Joueur]]:
        ...
        Les liberos peuvent être trouvés:
        1. Dans une ligne après le header "LIBEROS"
        2. Sur la même ligne que "LIBEROS" dans les colonnes suivantes
        3. Dans une cellule multi-ligne après le header
        """
        liberos_a = []
        liberos_b = []
        ...
        for table in tables:
            ...
            for row_idx, row in enumerate(table):
                ...
                if 'LIBEROS' in row_text:
                    ...
                    liberos_row_found = True
                    
                    # Vérifier les cellules sur la même ligne que LIBEROS
                    for i, cell in enumerate(row):
                        ...
                        # Skip le mot "LIBEROS" lui-même
                        if cell_str.upper() == 'LIBEROS':
                            continue
                        
                        # Essayer de parser les liberos dans cette cellule
                        # Peut contenir plusieurs joueurs séparés par \n
                        lines = cell_str.split('\n')
                        for line in lines:
                            line = line.strip()
                            match = self.PATTERNS['joueur_ligne'].match(line)
                ...
                if in_liberos_section and liberos_row_found:
                    # Parser les liberos de la ligne suivante si elle existe
                    for i, cell in enumerate(row):
                        if not cell:
                            continue
                    ...
                    # On a traité la ligne après LIBEROS, on sort
                    in_liberos_section = False
        
        return liberos_a, liberos_b
    
    def _mark_liberos(self, joueurs: list[Joueur], liberos: list[Joueur]) -> None:
        """Marque les joueurs comme liberos s'ils correspondent aux liberos trouvés."""
        libero_numeros = {l.numero for l in liberos}
        for joueur in joueurs:
            if joueur.numero in libero_numeros:
                joueur.est_libero = True
    
    def _mark_capitaine(self, joueurs: list[Joueur], capitaine_numero: str | None) -> None:
        """Marque le joueur comme capitaine s'il correspond au numéro trouvé."""
        if not capitaine_numero:
            return
        for joueur in joueurs:
            if joueur.numero == capitaine_numero:
                joueur.est_capitaine = True
                break
    
    def _parse_officiels_from_tables(self, tables: list) -> tuple[dict, dict]:
        """Parse les officiels (entraîneurs, assistants) depuis les tables."""
        officiels_a = {"entraineur": None, "assistant": None, "medecin": None}
        officiels_b = {"entraineur": None, "assistant": None, "medecin": None}
        ...
        for table in tables:
            ...
            for row in table:
                ...
                # Chercher les lignes d'officiels (Entraîneur, Assistant, Kinésithérapeute, Médecin)
                for role in ['Entraîneur', 'Entraineur', 'Assistant', 'Kinésithérapeute', 'Médecin']:
                    if role in row_text:
                        # Les officiels sont généralement dans une zone spécifique
                        # Pour l'instant on note juste qu'ils existent
                        pass
        
        return officiels_a, officiels_b
    
    def _parse_capitaines(self, tables: list, full_text: str) -> tuple[str | None, str | None]:
        """Parse les numéros des capitaines depuis les tables ou le texte.
        
        Les capitaines sont identifiés par:
        1. Un cercle autour de leur numéro dans la table des joueurs (difficile à parser)
        2. La section "Capitaine" dans la table des signatures
        3. Un marqueur spécial dans certains formats
        
        Returns:
            Tuple (numero_capitaine_a, numero_capitaine_b)
        """
        capitaine_a = None
        capitaine_b = None
        
        # Méthode 1: Chercher dans les tables une section "Capitaine"
        for table in tables:
            if not table:
                continue
            ...
            for i, row in enumerate(table):
                ...
                # Chercher une ligne avec "Capitaine"
                if 'Capitaine' in row_text and 'N°' in row_text:
                    # La ligne suivante pourrait contenir les numéros
                    if i + 1 < len(table):
                        next_row = table[i + 1]
                        if next_row:
                            # Chercher des numéros à 2 chiffres
                            for j, cell in enumerate(next_row):
                                if cell:
                                    cell_str = str(cell).strip()
                                    # Numéro de maillot (2 chiffres max)
                                    if cell_str.isdigit() and len(cell_str) <= 2:
                                        if j < len(next_row) // 2 and not capitaine_a:
                                            capitaine_a = cell_str.zfill(2)
                                        elif j >= len(next_row) // 2 and not capitaine_b:
                                            capitaine_b = cell_str.zfill(2)
        
        return capitaine_a, capitaine_b
    
    def _parse_sets_detailed(self, tables: list) -> list[dict]:
        """Parse les détails des sets (formations, changements, timeouts) depuis la table principale.
        
        Structure de la Table 0 (table principale):
        - Rows 0-10: Set 1
        - Rows 11-20: Set 2
        - Rows 21-29: Set 3
        - Rows 30+: Set 4, Set 5 (si nécessaire) et sanctions/arbitres
        
        Structure d'un set (offset dans le bloc du set):
        - Row +0: Headers (I, II, III, IV, V, VI pour les positions)
        - Row +1: "Ordre de Service" (ordre initial de rotation)
        - Row +2: "Formation de Départ" (positions des joueurs)
        - Row +3: "Remplaçants" (changements effectués)
        - Row +4-9: Tours de service et scores progressifs
        
        Returns:
            Liste de dicts avec les détails de chaque set
        """
        sets_detailed = []
        
        # Trouver la table principale (Table 0 généralement)
        main_table = None
        for table in tables:
            if table and len(table) >= 30:  # La table principale a ~44 lignes
                # Vérifier si c'est bien la table des sets
                first_row_text = ' '.join(str(c) for c in table[0] if c) if table[0] else ''
                # La table des sets commence souvent avec les numéros romains I-VI
                if any(x in first_row_text for x in ['I', 'II', 'III', 'Formation', 'Ordre']):
                    main_table = table
                    break
        
        if not main_table:
            return sets_detailed
        
        # Parser chaque set
        set_offsets = [0, 11, 21]  # Offsets approximatifs pour Sets 1, 2, 3
        
        for set_num, offset in enumerate(set_offsets, 1):
            if offset + 10 > len(main_table):
                break
            
            set_data = {
                "numero": set_num,
                "formation_a": None,  # Formation object pour équipe A
                "formation_b": None,  # Formation object pour équipe B
                "remplacements_a": [],
                "remplacements_b": [],
                "timeouts_a": [],
                "timeouts_b": [],
            ...
            # Row offset+2: Formation de Départ
            formation_row_idx = offset + 2
            if formation_row_idx < len(main_table):
                formation_row = main_table[formation_row_idx]
                if formation_row:
                    # Colonnes pour équipe A: indices ~1-6 (positions I-VI)
                    # Colonnes pour équipe B: indices ~8-13 (positions I-VI)
                    formation_a = self._extract_formation(formation_row, side='A')
                    formation_b = self._extract_formation(formation_row, side='B')
                    set_data["formation_a"] = formation_a
                    set_data["formation_b"] = formation_b
            
            # Row offset+3: Remplaçants
            remplacement_row_idx = offset + 3
            if remplacement_row_idx < len(main_table):
                remplacement_row = main_table[remplacement_row_idx]
                if remplacement_row:
                    set_data["remplacements_a"] = self._extract_remplacements(remplacement_row, side='A')
                    set_data["remplacements_b"] = self._extract_remplacements(remplacement_row, side='B')
            
            # Chercher les timeouts dans les lignes du set
            # Les timeouts sont marqués avec "T" et le score associé
            for row_idx in range(offset, min(offset + 10, len(main_table))):
                ...
                # Un timeout est souvent indiqué par une cellule "T" ou une durée
                timeouts = self._extract_timeouts_from_row(row)
                if timeouts:
                    for timeout in timeouts:
                        if timeout.get('side') == 'A':
                            set_data["timeouts_a"].append(timeout)
                        else:
                            set_data["timeouts_b"].append(timeout)
            
            sets_detailed.append(set_data)
        
        return sets_detailed
    
    def _extract_formation(self, row: list, side: str) -> Formation | None:
        """Extrait une formation depuis une ligne de tableau.
        
        Args:
            row: Liste des cellules de la ligne
            side: 'A' pour équipe A (colonnes gauche), 'B' pour équipe B (colonnes droite)
        
        Returns:
            Objet Formation ou None si pas de données valides
        """
        if not row:
            return None
        
        # Déterminer les indices de colonnes selon le côté
        # Structure typique: [label, pos1A, pos2A, pos3A, pos4A, pos5A, pos6A, ..., pos1B, pos2B, ...]
        # Les indices exacts dépendent du format du PDF
        
        positions = []
        
        # Filtrer les cellules non vides contenant des numéros
        cells_with_numbers = []
        for i, cell in enumerate(row):
            if cell:
                cell_str = str(cell).strip()
                # Un numéro de joueur est 1-2 chiffres
                if cell_str.isdigit() and len(cell_str) <= 2:
                    cells_with_numbers.append((i, cell_str))
        
        if len(cells_with_numbers) < 6:
            return None
        
        # Diviser entre équipe A et équipe B
        mid_idx = len(row) // 2
        
        if side == 'A':
            team_cells = [c for c in cells_with_numbers if c[0] < mid_idx][:6]
        else:
            team_cells = [c for c in cells_with_numbers if c[0] >= mid_idx][:6]
        ...
        try:
            return Formation(
                position_1=team_cells[0][1],
                position_2=team_cells[1][1],
                position_3=team_cells[2][1],
                position_4=team_cells[3][1],
                position_5=team_cells[4][1],
                position_6=team_cells[5][1]
            )
        except (IndexError, ValueError):
            return None
    
    def _extract_remplacements(self, row: list, side: str) -> list[dict]:
        """Extrait les remplacements depuis une ligne.
        
        Args:
            row: Liste des cellules de la ligne "Remplaçants"
            side: 'A' ou 'B'
        
        Returns:
            Liste de dicts avec {entrant, sortant, score}
        """
        remplacements = []
        ...
        # Les remplacements sont souvent notés: "X/Y" ou "X→Y" ou dans des cellules séparées
        for i, cell in enumerate(row):
            if not cell:
                continue
            
            # Vérifier si c'est du bon côté
            if (side == 'A' and i >= mid_idx) or (side == 'B' and i < mid_idx):
                continue
            
            cell_str = str(cell).strip()
            
            # Pattern: "X/Y" (entrant X remplace Y)
            import re
            match = re.match(r'(\d{1,2})[/→](\d{1,2})', cell_str)
            if match:
                remplacements.append({
                    "entrant": match.group(1).zfill(2),
                    "sortant": match.group(2).zfill(2),
                    "score": None
                })
        
        return remplacements
    
    def _extract_timeouts_from_row(self, row: list) -> list[dict]:
        """Extrait les timeouts depuis une ligne.
        
        Les timeouts sont marqués de différentes façons selon les PDFs:
        - Une cellule "T" avec le score à côté
        - Une indication de temps
        
        Returns:
            Liste de dicts avec {side, score_a, score_b}
        """
        timeouts = []
        ...
        # Chercher les cellules marquées "T" ou avec un pattern de timeout
        for i, cell in enumerate(row):
            if not cell:
                continue
            ...
            # Un timeout peut être marqué "T" ou un nombre avec apostrophe (durée)
            if cell_str == 'T':
                # Déterminer le côté basé sur la position
                side = 'A' if i < len(row) // 2 else 'B'
                
                # Chercher le score associé dans les cellules adjacentes
    ...
    def _parse_sets_from_tables(self, tables: list, resultat: dict, sets_detailed: list[dict] | None = None) -> list[Set]:
        ...
        if not sets and resultat.get("score_final"):
            try:
                ...
                # Créer des objets Set avec les détails si disponibles
                for i in range(sets_a + sets_b):
                    set_num = i + 1
                    
                    # Chercher les détails de ce set
                    set_details = None
                    if sets_detailed:
                        for sd in sets_detailed:
                            if sd.get("numero") == set_num:
                                set_details = sd
                                break
                    ...
                    if set_details:
                        formation_a = set_details.get("formation_a")
                        formation_b = set_details.get("formation_b")
                        
                        # Convertir les timeouts dicts en objets TimeOut
                        # TimeOut requiert score_a et score_b, donc on ne crée que si les deux sont présents
                        for t in set_details.get("timeouts_a", []):
                            score_a = t.get("score_a")
                            score_b = t.get("score_b")
                            if score_a is not None and score_b is not None:
                                try:
                                    timeouts_a.append(TimeOut(
                                        score_a=score_a,
                                        score_b=score_b
                                    ))
                                except Exception:
                        ...
                        for t in set_details.get("timeouts_b", []):
                            score_a = t.get("score_a")
                            score_b = t.get("score_b")
                            if score_a is not None and score_b is not None:
                                try:
                                    timeouts_b.append(TimeOut(
                                        score_a=score_a,
                                        score_b=score_b
                                    ))
                                except Exception:
                    ...
                    set_obj = Set(
                        numero=set_num,
                        score_a=None,  # On ne connaît pas les scores exacts des sets
                        score_b=None,
    ...
    def _parse_sanctions(self, tables: list, words: list) -> list[Sanction]:
        ...
        Les sanctions sont dans la Table 0, généralement après les sets (rows 30+).
        Structure:
        - Row 30: Header "SANCTIONS N° Set Faute Score Carton"
        - Row 31-33: Données de sanctions pour chaque équipe
        
        Types de sanctions:
        - Avertissement (carton jaune)
        - Pénalité (carton rouge)
        - Expulsion (carton rouge + jaune)
        - Disqualification (double carton rouge)
        
        Returns:
            Liste d'objets Sanction
        """
        sanctions = []
        
        # Chercher la table principale contenant les sanctions
        for table in tables:
            if not table or len(table) < 30:
                continue
            
            in_sanctions_section = False
            current_equipe = 'A'  # Alternera entre A et B
            ...
            for row_idx, row in enumerate(table):
                ...
                # Détecter l'en-tête des sanctions
                if 'SANCTIONS' in row_text or ('FAUTE' in row_text and 'CARTON' in row_text):
                    in_sanctions_section = True
                    continue
                
                if in_sanctions_section:
                    # Les lignes de sanctions contiennent:
                    # - N° du joueur
                    # - Set où la sanction a eu lieu
                    # - Type de faute
                    # - Score au moment de la sanction
                    # - Type de carton (J=jaune, R=rouge)
                    ...
                    for cell_idx, cell in enumerate(row):
                        ...
                        # Déterminer l'équipe basée sur la position dans la ligne
                        mid_idx = len(row) // 2
                        current_equipe = 'A' if cell_idx < mid_idx else 'B'
                        
                        # Parser les données de sanction
                        # Format possible: "03 2 Conduite 15-12 J" ou dans des cellules séparées
                        
                        # Pattern pour sanction complète dans une cellule
                        import re
                        match = re.search(
                            r'(\d{1,2})\s+(\d)\s+[\w\s]*?(\d+)-(\d+)\s*([JRjr])',
                            cell_str
                        )
                        if match:
                            numero_joueur = match.group(1).zfill(2)
                            set_num = int(match.group(2))
                            score_a = int(match.group(3))
                            score_b = int(match.group(4))
                            carton = match.group(5).upper()
                            
                            # Déterminer le type de sanction
                            type_sanction = TypeSanction.AVERTISSEMENT
                            if carton == 'R':
                                type_sanction = TypeSanction.PENALITE
                    ...
                    # On ne reste pas trop longtemps dans la section sanctions
                    if row_idx > 35:
                        break
