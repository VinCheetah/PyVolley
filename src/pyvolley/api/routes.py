...
async def get_joueur_match_detailed_stats(
    ...
    joueurs_remplaces: Optional[str] = Query(None, description="Numéros séparés par des virgules"),
    joueur_repo: JoueurRepository = Depends(get_joueur_repo),
    match_repo: MatchRepository = Depends(get_match_repo),
):
    """Statistiques détaillées d'un joueur sur un match.

    Supporte le mode libéro et le remplacement par libéro.
    """
    ...
    participants_a = [p for p in (match_db.participations or []) if p.equipe_id == match_db.equipe_a_id]
    participants_b = [p for p in (match_db.participations or []) if p.equipe_id == match_db.equipe_b_id]

    match_core = match_db_to_core(match_db, participants_a, participants_b)

    remplaces = (
        [n.strip() for n in joueurs_remplaces.split(",") if n.strip()]
        if joueurs_remplaces else None
    )
    ...
    if not stats:
        raise HTTPException(status_code=404, detail="Joueur non trouvé dans ce match")
