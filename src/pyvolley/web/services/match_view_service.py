...
class MatchViewService:
    ...
    def build_detail_context(cls, match_id: int, session: Session) -> Optional[dict[str, Any]]:
        ...
        for side, parts in [("A", participants_a), ("B", participants_b)]:
            for p in parts:
                num = normalize_numero(p.numero_maillot)
                p_dict = {
                    "numero": p.numero_maillot,
                ...
                if num:
                    players_by_side_num[side][num] = p_dict
                if p.est_libero and not libero_by_side[side]:
                    libero_by_side[side] = p_dict

        # Dictionnaires de formation tactique par set
        set_courts: dict[int, dict[str, Any]] = {}
        for s in (match.sets or []):
            set_courts[s.numero] = {"A": None, "B": None}
            for f in (s.formations or []):
                side = f.equipe
                if side not in ("A", "B"):
                    continue
                formation_positions: dict[int, dict[str, Any]] = {}
                for pos_idx in range(1, 7):
                    jersey_str = getattr(f, f"position_{pos_idx}", None)
                    if jersey_str:
                        norm = normalize_numero(jersey_str)
                        if norm and norm in players_by_side_num[side]:
                            formation_positions[pos_idx] = players_by_side_num[side][norm]
                        else:
                            formation_positions[pos_idx] = {
                                "numero": jersey_str,
                                "display_name": f"#{jersey_str}",
                                "joueur_id": None,
                            }
                set_courts[s.numero][side] = {
                    "positions": formation_positions,
                    "libero": libero_by_side[side],
                }
