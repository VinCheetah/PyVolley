...
def test_compare_help():
    """Vérifie que la commande dev compare --help s'exécute correctement sans erreur."""
    result = runner.invoke(app, ["dev", "compare", "--help"])
...
def test_compare_invalid_parser():
    ...
    result = runner.invoke(app, ["dev", "compare", "-p", "unknown_parser"])
...
def test_compare_custom_parsers_flags(tmp_path):
    ...
    result = runner.invoke(app, ["dev", "compare", str(tmp_path), "-p", "legacy", "-p", "fast"])
    assert result.exit_code == 0
...
def test_compare_custom_parsers_comma_separated(tmp_path):
    ...
    result = runner.invoke(app, ["dev", "compare", str(tmp_path), "-p", "legacy,fast"])
    assert result.exit_code == 0
    assert "Aucun fichier PDF trouvé" in result.output


def test_compare_execution_with_pdf():
    """Vérifie le comportement de dev compare lorsqu'un vrai PDF est fourni."""
    sample_pdf = Path("data/data_sample/BMAA003.pdf")
    if not sample_pdf.exists():
        pytest.skip("Fichier de test data/data_sample/BMAA003.pdf introuvable.")

    result = runner.invoke(app, ["dev", "compare", str(sample_pdf), "-p", "legacy", "-p", "fast", "-v"])
    assert result.exit_code == 0
